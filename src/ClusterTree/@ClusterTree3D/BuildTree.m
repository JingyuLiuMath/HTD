...
if T.B_.size_ <= min_points
    ...
    for it3 = 1 : 2
        for it2 = 1 : 2
            for it1 = 1 : 2
                T.ch_{it1, it2, it3} = ClusterTree3D(...
                    B_children{it1, it2, it3}, ...
                    T.level_ + 1);
                T.ch_{it1, it2, it3}.BuildTree(min_points);
                T.ind_ = [T.ind_; T.ch_{it1, it2, it3}.ind_];
                T.max_level_ = max(T.max_level_, ...
                    T.ch_{it1, it2, it3}.max_level_);
            end
        end
    end
end
